	...
	<div class="main">
		...
		<%
			request.getParameter("utf-8");
				...
				if (pages == null) {
					response.sendRedirect("../Vnasi_newsServlet?opr=list&pageNo=1");
				} else {
			...
			<table class="list">
				...
				<c:forEach items="${sessionScope.pa.getNewsList() }" var="news1" varStatus="Status">
							...
							<tr>
								...
								<td class="w1 c"><a href="../Vnasi_newsServlet?opr=getNewsById&baiId=${news1.en_id }" >修改</a>
								<a class="manageDel" href="javascript:if(confirm('是否确认删除？')){location.href='../Vnasi_newsServlet?opr=delete&id=${news1.en_id}';};">删除</a>
								</td>
							</tr>
						</c:forEach>
			</table>
				...
				<ul class="clearfix">
					<li  class="current">[<%=pageNo%>/<%=totalPageCount%>]
							...
							<%
								if (pageNo > 1) {
							%>
							<li><a href="../Vnasi_newsServlet?opr=list&pageNo=1">首页</a></li>
							<li><a href="../Vnasi_newsServlet?opr=list&pageNo=<%=pageNo - 1%>">上一页</a></li>
							<%
								}
								if (pageNo < totalPageCount) {
							%>
							<li><a href="../Vnasi_newsServlet?opr=list&pageNo=<%=pageNo + 1%>">下一页</a></li>
							<li><a
								href="../Vnasi_newsServlet?opr=list&pageNo=<%=totalPageCount%>">末页</a></li>
							<%
								}
							%>
				</ul>
