...
<%
	...
	if (pages == null) {
		//说明是用户直接打开页面，默认打开第一页
		response.sendRedirect("<%=request.getContextPath()%>/CommentServlet?opr=list&pageIndex=1");
	} else {
				...
				<dl>
					...
					<dd>
						<em><a href="<%=request.getContextPath()%>/ProductServlet?opr=selectFu">新增</a></em><a
							href="<%=request.getContextPath()%>/ProductServlet?opr=list">分类管理</a>
					</dd>
					<dd>
						<em><a href="<%=request.getContextPath()%>/ShoppingServlet?opr=insert">新增</a></em><a href="product.jsp">商品管理</a>
					</dd>
					<dt>订单管理</dt>
				...
				<table class="list">
					...
					<tr>
						...
						<td>
							...
							%>
								<a href="<%=request.getContextPath()%>/CommentServlet?opr=hui&id=<%=use.getId()%>">	
									回复
								</a>
							<%
							}else{
							%>
								<a href="<%=request.getContextPath()%>/CommentServlet?opr=hui&id=<%=use.getId()%>">修改</a>
							<%} %>
							&nbsp;<a href="javascript:if(confirm('是否确认删除？')){location.href='<%=request.getContextPath()%>/CommentServlet?opr=delete&id=<%=use.getId()%>';};" href="">删除</a>
							</td>
					</tr>
					<%
						}
				...
				<div class="pager">
					<ul class="clearfix">
						...
						<li>
							<%
								if (pageNo > 1) {
							%> <a href="<%=request.getContextPath()%>/CommentServlet?opr=list&pageIndex=1">首页</a> <a
							href="<%=request.getContextPath()%>/CommentServlet?opr=list&pageIndex=<%=pageNo - 1%>">上一页</a>
							 <%
							 	}
							 %>
						</li>
    					...
    					if(pageNo<pagesCount){
   				 		%>
						<li><a href="<%=request.getContextPath()%>/CommentServlet?opr=list&pageIndex=<%=pageNo+1%>">下一页</a></li>
						<li><a href="<%=request.getContextPath()%>/CommentServlet?opr=list&pageIndex=<%=pagesCount%>">末页</a></li>
