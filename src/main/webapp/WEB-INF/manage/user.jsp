					...
					<tr>
						...
						<td class="w1 c">
							<c:if test="${currentUser.status==2 }">
									<c:if test="${list1.status==1 }">   <a href="javascript:if(confirm('是否确认删除？')){location.href='<%=request.getContextPath()%>/UserServlet?opr=delete&id=${list1.id }';};">删除</a></c:if>
									<c:if test="${list1.status==2 }"> <a class="manageDel" href="<%=request.getContextPath()%>/UserServlet?opr=update&uid=${list1.id }">修改</a></c:if>
							</c:if>
							<c:if test="${currentUser.status==1 }">
									
									 <a class="manageDel" href="<%=request.getContextPath()%>/UserServlet?opr=update&uid=${list1.id }">修改</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
