...
<%@page import="cn.vnasi.entity.Product_category"%>
...
function checkClassName() {
    ...
    if (className == "") {
    	classNameA.html("分类不能为空");
    	/*alert("密码不能为空");*/
        return false;
    }
    classNameA.html("可以使用").addClass("ok_prompt");
    return true;
}
			...
			<form action="<%=request.getContextPath()%>/ProductServlet?opr=updateFZ&id=${category.epc_id }" method="post" id="zong">
				<table class="form">
						...
						<td class="field">父分类：</td>
							...
							<select name="parentId">
								...
								<c:forEach var="list" items="${categoryList }" varStatus="sta">
									<c:if test="${category.epc_id ne category.epc_parent_id }"><c:if test="${list.epc_id eq list.epc_parent_id }"><option value="${list.epc_id }">${list.epc_name }</option></c:if></c:if>
								</c:forEach>
							</select>
						</td>
					...
					<tr>
						...
						<td><input type="text" class="text" name="className" value="${category.epc_name }" id="className" /></td>
