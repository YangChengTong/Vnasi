...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>商品管理 </h2><br/>
		<form action="../ShoppingServlet?opr=list&pageNo=1" method="post" >&nbsp;&nbsp;&nbsp;可模糊查询:<input type="text" name="textShoppingName" value="${sessionScope.houtaiShopping }" /> <input type="submit"  value="查询"/></form>
		<div class="manage">
			<table class="list">
				<tr>
					<th>编号</th>
			  ...
			  <%
				...
				if (pages == null) {
					response.sendRedirect("../ShoppingServlet?opr=list&pageNo=1");
			   ...
			   <c:forEach var="list" items="${shoppingPage.topicShoopList }">
			   	<tr>
					...
					<td class="thumb"><img src="../files/${list.ep_file_name }" width="50" height="50" /><a href="#" target="_self">${list.ep_name } </a></td>
					<td class="w1 c"><a href="../ShoppingServlet?opr=selectById&id=${list.ep_id}">修改</a> <a class="manageDel" href="javascript:if(confirm('是否确认删除？')){location.href='../ShoppingServlet?opr=delete&id=${list.ep_id}';};">删除</a></td>
				</tr>
			   </c:forEach>
				...
				<ul class="clearfix">
					<li  class="current">[<%=pageNo%>/<%=totalPageCount%>]
							...
							<%
								if (pageNo > 1) {
							%>
							<li><a href="../ShoppingServlet?opr=list&pageNo=1&textShoppingName=${houtaiShopping }">首页</a></li>
							<li><a href="../ShoppingServlet?opr=list&textShoppingName=${houtaiShopping }&pageNo=<%=pageNo - 1%>">上一页</a></li>
							<%
								}
								if (pageNo < totalPageCount) {
							%>
							<li><a href="../ShoppingServlet?opr=list&textShoppingName=${houtaiShopping }&pageNo=<%=pageNo + 1%>">下一页</a></li>
							<li><a
								href="../ShoppingServlet?opr=list&textShoppingName=${houtaiShopping }&pageNo=<%=totalPageCount%>">末页</a></li>
							<%
								}
							%>
				</ul>
