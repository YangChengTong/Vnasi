			...
			<table class="list">
				...
				<c:forEach var="list" items="${categoryList }" varStatus="sta">
					<tr <c:if test="${list.epc_id eq list.epc_parent_id}">style="background-color:#FFCCFF"</c:if>>
						...
						<td class="w1 c"><a href="<%=request.getContextPath()%>/ProductServlet?opr=selectID&id=${list.epc_id }&name=${list.epc_name}">修改</a> <a class="manageDel" href="javascript:if(confirm('是否确认删除？')){location.href='<%=request.getContextPath()%>/ProductServlet?opr=delete&id=${list.epc_id }';};">删除</a></td>
					</tr>
				</c:forEach>
